...
public:
	...
	Test_data_set(BasicDomain& bd) {
		...
		//parameters
		double phase = 0.0, k = 400.0, c = 3.0e8, epsi = 8.854187817e-12, miu = 4 * 3.14159265358979*1e-7, ita = sqrt(epsi / miu);
		double A = 100;
		V3d vx{ 1.0,0.0,0.0 };
		V3d vy{ 0.0,1.0,0.0 };
		V3d vz{ 0.0,0.0,1.0 };
		...
		for (long i = 0; i < Ne; i++) {
			...
			Phaser Ez = A * exp(-k * para.position.y * 1.0i + phase * 1.0i);
			Phaser curlh_z = k * 1.0i * A * ita * exp(-k * para.position.y * 1.0i + phase * 1.0i);
			if (para.shader == BOUNDARY) {
				e_b[i] = para.direction*vz*Ez;
			}
			else {
				e_in[i] = para.direction*vz*Ez;
				curlh_in[i] = para.direction*vz*curlh_z;
			}
		}
		//set h_in & curle
		for (long i = 0; i < Nh; i++) {
			Parameter para = bd.get_Hparas(i);
			Phaser Hx = A * ita * exp(-k * para.position.y * 1.0i + phase * 1.0i);
			Phaser curle_x = -k * 1.0i * A * exp(-k * para.position.y * 1.0i + phase * 1.0i);
			if (para.shader == BOUNDARY) {
				h_b[i] = para.direction*vx*Hx;
				curle_b[i] = para.direction*vx*curle_x;
			}
			else {
				h_in[i] = para.direction*vx*Hx;
				curle_in[i] = para.direction*vx*curle_x;
			}
		}
	}
